...
function crearTarea() {
  ...
  if (!title || isNaN(user_id)) {
    const result = document.getElementById('task-result');
    result.textContent = '❌ Error: Título y ID de usuario son requeridos';
    result.className = 'error';
    return;
  }
  ...
  .then(data => {
    ...
    if (data.id) {
      ...
      result.className = 'result';
      verTareas();  // Actualizar la lista de tareas automáticamente
    } else {
      result.textContent = `❌ Error: ${data.error || 'Error desconocido'}`;
      result.className = 'error';
    }
  })
  .catch(error => {
