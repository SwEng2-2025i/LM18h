  ...
  fetch('http://localhost:5001/users')
    ...
    .then(data => {
      ...
      data.forEach(u => {
        ...
        li.innerHTML = `${u.name} (ID: ${u.id}) <button onclick="eliminarUsuario(${u.id})">Eliminar</button>`;
        ul.appendChild(li);
      });
    });
}
...
function verTareas() {
  fetch('http://localhost:5002/tasks')
    ...
    .then(data => {
      ...
      data.forEach(t => {
        let li = document.createElement('li');
        li.innerHTML = `${t.title} (Usuario ID: ${t.user_id}) <button onclick="eliminarTarea(${t.id})">Eliminar</button>`;
        ul.appendChild(li);
      });
    });
}
