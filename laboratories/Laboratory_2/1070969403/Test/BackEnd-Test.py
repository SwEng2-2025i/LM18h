...
def delete_user(user_id):
    ...
    if response.status_code == 200:
        ...
        print(f" Could not delete user {user_id} (may not exist). Status: {response.status_code}")
...
def delete_task(task_id):
    ...
    if response.status_code == 200:
        ...
        print(f"Could not delete task {task_id} (may not exist). Status: {response.status_code}")
    return response.status_code == 200
...
def generate_pdf_report(lines):
    ...
    c.drawString(40, y, f"BackEndIntegration Test Report #{next_num}")
    y -= 30
    for line in lines:
