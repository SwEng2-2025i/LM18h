...
def delete_tasks_by_user(user_id):
    """Delete all tasks for a specific user - used for test cleanup"""
    try:
        num_deleted = Task.query.filter_by(user_id=user_id).delete()
        db.session.commit()
        return jsonify({'message': f'All tasks for user {user_id} deleted successfully', 'count': num_deleted}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500
